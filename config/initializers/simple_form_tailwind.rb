...
SimpleForm.setup do |config|
  ...
  config.label_class = 'text-base font-semibold text-black'


  # vertical forms
  #
  # vertical default_wrapper
  config.wrappers :vertical_form, tag: 'div', class: 'mb-4 mt-4' do |b|
    b.use :html5
    b.use :placeholder
    ...
    b.optional :min_max
    b.optional :readonly
    b.use :label, class: 'block mt-4 mb-1', error_class: 'text-red-500'
    b.use :input, class: 'shadow appearance-none border border-gray-200 bg-gray-100 rounded-lg text-m w-full mt-2 py-2 px-3 text-gray-400 leading-6 transition-colors duration-200 ease-in-out', error_class: 'border-red-500', valid_class: 'border-green-400'
    b.use :full_error, wrap_with: { tag: 'p', class: 'mt-2 text-red-500 text-s italic' }
    b.use :hint, wrap_with: { tag: 'p', class: 'mt-2 text-black text-[14px] font-normal' }
  end
  ...
  config.wrappers :vertical_boolean, tag: 'div', class: 'mb-4 flex items-start', error_class: '' do |b|
    ...
    b.wrapper tag: 'div', class: 'ml-3 text-sm' do |bb|
      ...
      bb.use :hint, wrap_with: { tag: 'p', class: 'block text-grey-700 text-s italic' }
      bb.use :full_error, wrap_with: { tag: 'p', class: 'block text-red-500 text-s italic' }
    end
  ...
  config.wrappers :vertical_collection, item_wrapper_class: 'flex items-center', item_label_class: 'my-1 ml-3 block text-sm font-medium text-gray-400', tag: 'div', class: 'my-4' do |b|
    ...
    b.use :full_error, wrap_with: { tag: 'p', class: 'block mt-2 text-red-500 text-s italic' }
    b.use :hint, wrap_with: { tag: 'p', class: 'mt-2 text-grey-700 text-s italic' }
  end
  ...
  config.wrappers :vertical_multi_select, tag: 'div', class: 'mt-4', error_class: 'f', valid_class: '' do |b|
    b.use :html5
    b.optional :readonly
    b.wrapper :legend_tag, tag: 'legend', class: 'text-sm font-medium text-gray-600', error_class: 'text-red-500' do |ba|
      ba.use :label_text
